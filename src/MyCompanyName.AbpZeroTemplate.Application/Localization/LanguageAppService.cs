...
namespace MyCompanyName.AbpZeroTemplate.Localization
{
    ...
    public class LanguageAppService : AbpZeroTemplateAppServiceBase, ILanguageAppService
    {
        ...
        {
            var languages = (await _applicationLanguageManager.GetLanguagesAsync(AbpSession.TenantId)).OrderBy(l => l.Id);
        ...
        public async Task<GetLanguageForEditOutput> GetLanguageForEdit(NullableIdDto input)
        {
            ...
            //Flags
            output.Flags = FlagIconsHelper
                .FlagClassNames
                .OrderBy(f => f)
                .Select(f => new ComboboxItemDto(f, FlagIconsHelper.GetCountryCode(f)) { IsSelected = output.Language.Icon == f })
                .ToList();
